#include <Arduino.h>
#include <Servo.h>
const int PAN = 6;         //Pan Servo Digital Pin
const int TILT = 8;        //Tilt Servo Digital Pin
const int LASER = 2;        //Laser Digital Pin
const int PAN_MIN = 50;     // minimum pan servo range, value should be between 0 and 180 and lower than PAN_MAX
const int PAN_MAX = 140;    // maximum pan servo range, value should be between 0 and 180 and more than PAN_MIN
const int TILT_MIN = 80;    // minimum tilt servo range, value should be between 0 and 180 and lower than TILT_MAX
...
const int SPEED_MAX = 15;   // maximum interpolation speed (Lower SPEED_MAX values mean that the program will cycle more quickly, leading to an increased occurance of pausing. Adjust ROLL_MAX accordingly)
const int ROLL_MAX = 40;    // Roll the dice for random pause. Lower value = more frequent pauses, higher value = less frequent pauses
const int PAUSE_MIN = 100;  // define minimum pause length in mS. 1000 = 1 second
const int PAUSE_MAX = 500; // define maximum pause length in mS.
